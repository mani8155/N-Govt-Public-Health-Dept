...
import json
from vas1.mbbs_proforma import ApplicationTableName, get_file_psk_id, ApplicationStudentTable, ApplicationPHCTable, \
    ApplicationCheckListTable, insert_application_phc
from vas1.nursing_proforma import nursing_student_data_get


def anm_application_navigation(request):
    ...
    for check in check_app:
        ...
        if course_master_obj and course_master_obj.get('course_type') == "ANM":
            return redirect('anm_update', check['psk_id'])

    # If no application matches the condition, redirect to 'anm_application'
    return redirect('anm_application')
...
def anm_application(request):
    ...
    course_data = institution.course_data_get(_type="ANM")
    ...
    if request.method == "POST":
        ...
        # 4.Copy of the List of Recognized Colleges in the Tamil Nadu Nurses and Midwives Council Web site (Evidence
        # Produced)
        ...
        # 9. No due certificate for previous batch internship fees
        no_dues_certificate_upload_uid = request.FILES.get("no_dues_certificate_upload_uid")

        # 10. Consent Letter for 1000 Sq.Ft building
            ...
            for field_name, upload_file in file_fields:
                if upload_file and upload_file.size > MAX_FILE_SIZE:
                    messages.info(request, "Oops! Your file is too large (max 2MB). Please choose a smaller PDF "
                                           "and try again.")
                    return redirect('anm_update', rec_parent_id)
    ...
    return render(request, "proforma/anm/anm_application.html", context)


def anm_change_course(request, psk_id):
    ...
    return redirect('anm_student', psk_id)


def anm_update(request, psk_id):
    saved_username = request.COOKIES.get('saved_username', None)
    applicant_data = get_applicant_user_data(request, username=saved_username)
    institution_id = applicant_data.get('institution_name')
    institution_obj = institution.institution_id_data_get(int(institution_id))
    course_data = institution.course_data_get("ANM")
    company = institution.company_master_data_get()[0]
    ...
    if request.method == "POST":
        ...
        required_files = [
            ...
            "recognized_colleges_list_tn_upload_uid",
        ...
        if missing_files:
            ...
            return redirect('anm_update', psk_id)
        ...
        if response.status_code == 200:
            ...
            for field in file_fields:
                ...
                if uploaded_file:
                    ...
                    if uploaded_file.content_type == "application/pdf" and uploaded_file.size > MAX_FILE_SIZE:
                        messages.info(request, "Oops! Your file is too large (max 2MB). Please choose a smaller PDF "
                                               "and try again.")
                        return redirect('anm_update', psk_id)  # Skip processing this file
            ...
            return redirect('anm_student', parent_id)
    ...
    return render(request, "proforma/anm/anm_update.html", context)


def delete_anm_student(request, psk_id, rec_parent_id):
    url = f"{API_STUDIO_URL}deleteapi/delete/phpm02_application_302_student_dc2/{psk_id}"
    response = requests.delete(url)

    if response.status_code == 200:
        messages.success(request, message="Deleted Successfully")
        redirect_url = 'anm_student'
    else:
...
def anm_student(request, rec_parent_id):
    saved_username = request.COOKIES.get('saved_username', None)
    ...
    return render(request, "proforma/anm/anm_student.html", context)


def delete_anm_phc(request, psk_id, rec_parent_id):
...
def anm_phc_form(request, rec_parent_id):
    saved_username = request.COOKIES.get('saved_username', None)
    ...
    if request.method == 'POST':
        ...
        if check_phc['phc_status'] == "UN-ALLOCATED":
            ...
            if response.status_code == 200:
                ...
                messages.success(request, message='Data Added successfully!')
                return redirect('anm_phc_form', rec_parent_id=rec_parent_id)
    ...
    return render(request, "proforma/anm/anm_phc_form.html", context)


def anm_check_list(request, rec_parent_id):
    saved_username = request.COOKIES.get('saved_username', None)
    applicant_data = get_applicant_user_data(request, username=saved_username)
    ...
    if request.method == 'POST':
        ...
        if final_check_result.get("final_check") is not True:
            ...
            return redirect('anm_check_list', rec_parent_id)
    ...
    return render(request, 'proforma/anm/anm_check_list.html', context)
