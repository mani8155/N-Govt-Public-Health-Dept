...
def mphw_application_navigation(request):
    ...
    for check in check_app:
        ...
        if course_master_obj and course_master_obj.get('course_type') == "MPHW(M)":
            return redirect('mphw_update', check['psk_id'])

    # If no application matches the condition, redirect to 'anm_application'
    return redirect('mphw_application')
...
def mphw_application(request):
    ...
    course_data = institution.course_data_get(_type="MPHW(M)")
    ...
    if request.method == "POST":
        ...
        # 7. No due certificate for previous batch internship fees
        no_dues_certificate_upload_uid = request.FILES.get("no_dues_certificate_upload_uid")

        # 8. Consent Letter for 1000 Sq.Ft building
        ...
        if response.status_code == 200:
            ...
            for field_name, upload_file in file_fields:
                if upload_file and upload_file.size > MAX_FILE_SIZE:
                    messages.info(request, "Oops! Your file is too large (max 2MB). Please choose a smaller PDF "
                                           "and try again.")
                    return redirect('mphw_update', rec_parent_id)
            ...
            return redirect('mphw_student', rec_parent_id)

    context = {"obj": institution_obj, "course_data": course_data, "company": company, "current_date": current_date}
    return render(request, "proforma/mphw/mphw_application.html", context)


def mphw_change_course(request, psk_id):
    ...
    return redirect('mphw_student', psk_id)


def mphw_update(request, psk_id):
    saved_username = request.COOKIES.get('saved_username', None)
    applicant_data = get_applicant_user_data(request, username=saved_username)
    institution_id = applicant_data.get('institution_name')
    institution_obj = institution.institution_id_data_get(int(institution_id))
    course_data = institution.course_data_get("MPHW(M)")
    company = institution.company_master_data_get()[0]
    ...
    if request.method == "POST":
        ...
        if missing_files:
            ...
            return redirect('mphw_update', psk_id)
        ...
        if response.status_code == 200:
            ...
            file_fields = [
                "gov_order_upload_uid", "permitted_in_phc_upload_uid",
                "pay_fees_by_upload_uid", "own_hospital_mou_upload_uid",
            ...
            for field in file_fields:
                ...
                if uploaded_file:
                    ...
                    if uploaded_file.content_type == "application/pdf" and uploaded_file.size > MAX_FILE_SIZE:
                        messages.info(request, "Oops! Your file is too large (max 2MB). Please choose a smaller PDF "
                                               "and try again.")
                        return redirect('mphw_update_update', psk_id)  # Skip processing this file
            ...
            return redirect('mphw_student', parent_id)
    ...
    return render(request, "proforma/mphw/mphw_update.html", context)


def delete_mphw_student(request, psk_id, rec_parent_id):
    url = f"{API_STUDIO_URL}deleteapi/delete/phpm02_application_302_student_dc2/{psk_id}"
    response = requests.delete(url)

    if response.status_code == 200:
        messages.success(request, message="Deleted Successfully")
        redirect_url = 'mphw_student'
    else:
...
def mphw_student(request, rec_parent_id):
    ...
    if request.method == 'POST':
        ...
        if response.status_code == 200:
            ...
            return redirect('mphw_student', rec_parent_id=rec_parent_id)
...
def delete_mphw_phc(request, psk_id, rec_parent_id):
...
def mphw_phc_form(request, rec_parent_id):
    ...
    if request.method == 'POST':
        ...
        if check_phc['phc_status'] == "UN-ALLOCATED":
            ...
            if response.status_code == 200:
                ...
                messages.success(request, message='Data Added successfully!')
                return redirect('mphw_phc_form', rec_parent_id=rec_parent_id)
    ...
    return render(request, "proforma/mphw/mphw_phc_form.html", context)
...
def mphw_check_list(request, rec_parent_id):
    ...
    if request.method == 'POST':
        ...
        if final_check_result.get("final_check") is not True:
            ...
            return redirect('mphw_check_list', rec_parent_id)
    ...
    return render(request, 'proforma/mphw/mphw_check_list.html', context)
